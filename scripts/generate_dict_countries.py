# Generate ordered Ordereddict for config.py
...
dict_ordered = {}
for each_item in countries:
    dict_ordered[each_item["code"]] = {
        "continent": each_item["continent"],
        "name": each_item["name"]
    }
dict_ordered_sorted = OrderedDict(sorted(dict_ordered.items(), key=lambda x: x[1]['name']))

print("countries = OrderedDict([")
for code in dict_ordered_sorted:
    print('    ("{}", {{"name": "{}", "continent": "{}"}}),'.format(
        code,
        dict_ordered[code]["name"],
        dict_ordered[code]["continent"]))
print("])")
